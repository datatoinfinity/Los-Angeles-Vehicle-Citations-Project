...
# stored as a Space Id in the Parking Meter data, and a Meter Id in the Citations
# Data. Normally, the Parking Meter data stored the meter as 2 alphabet letters
# followed by the meter number (for example, WW240). However, in the Citations
# Data many meters were stored without the letters, and thus could not be
# matched accurately. Potentially we could estimate a meter match to a citation
# based on the latitude and longitude data, but upon cursory inspection it
# apperas that the coordinates for the citation do not match the coordinates
# for the meter.
all_data <- clean_data %>%
...
fwrite(all_data, "Cleaned Data.csv", dateTimeAs = "write.csv")
```

# Remove Unecessary Objects

```{r}
rm(raw_data, projections, proj4string, new_coords, meter_data, clean_data,
   clean_meters)
```
