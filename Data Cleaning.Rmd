...
# Change Date-Time Variables to Character Before Fwrite

```{r}
# Fwrite writes data fast, but changes date-time variables into ISO format.
# Will store as character before writing to preserve data.
#clean_data$Issue_DT <- as.character(clean_data$Issue_DT)
#clean_data$TOD <- as.character(clean_data$TOD)
```

# Write Excel Spreadsheets

```{r}
# For date-time columns, will be rewritten in ISO format. Will transform back.
