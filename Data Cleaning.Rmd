...
# length(unique(raw_data$`Ticket number`)) == nrow(raw_data)
```
...
clean_data$Issue_DT <- apply(clean_data[, c('Issue_Date','Issue_Time')], 1,
                             paste, collapse = " ") %>%
  # Some values in the raw data were missing Issue Time (NA), so were quieted
  # to not show in output.
  ymd_hm(., quiet = TRUE)
# Change timezone from UTC.
tz(clean_data$Issue_DT) <- 'America/Los_Angeles'
# Now remove unnecessary columnss.
...
fwrite(cit_coords, "Coordinates to Plot.csv")
```
