...
ggplot(cits_type_by_car, aes(x = Make, y = Total_Cits,
  ...
  labs(title = "Most Common Type of Violation By Make for 2018",
       x = "Top 8 Car Makes", y = "Total # of Citations",
...
ggplot(data = tot_meter_cits, aes(Total_Cits)) +
  ...
  geom_text(aes(x = median(tot_meter_cits$Total_Cits) + 8, y = 1800,
                label = paste("Median of",
                              round(median(tot_meter_cits$Total_Cits),
                                    2), "")),
            colour = "black") +  
...
ggmap(la) + 
  ...
  geom_point(data = worst_meters, aes(x = Approx_Lon, y = Approx_Lat,
                                      color = Time_Limit),
             alpha = 1, size = 1.5) +
  labs(title = "Top 5% of Meters for Total Citations in 2018",
...
#a <- clean_data %>%
#  filter(Weekday == "Monday" & TOD >= as.POSIXct("2019-03-29 08:00:00", tz = "America/Los_Angeles") & TOD <= as.POSIXct("2019-03-29 09:00:00", #tz = "America/Los_Angeles")) %>%
#  group_by(Date) %>%
#  summarise(Tot_Cits = n()) 
#median(a$Tot_Cits)
#b <- clean_data %>%
#  filter(!(Month == "")) %>%
#  group_by(Month) %>%
#  summarise(Total_Cits = n())
#b$Month <- factor(b$Month, levels = c("Jan", "Feb", "Mar", "Apr", "May", "Jun",
#                                      "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"))
#b$Holidays <- as.numeric(list(0, 0, 1, 1, 2, 1, 0, 0, 1, 1, 0, 1))
#b$Num_Days <- as.numeric(list(30, 31, 31, 28, 31, 31, 30, 31, 31, 30, 31, 30))
#model <- lm(Total_Cits ~ Holidays + Num_Days, data = b)
#summary(model)
# Only 6% of the varation in the number of total citations in a month can be
# explained by that month's number of day and number of federal holidays.
```
